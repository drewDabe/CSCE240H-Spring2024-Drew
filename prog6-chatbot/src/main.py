...
class Query:
    ...
    def get_min_query_phrase(self, s):
        self.query_value = 9223372036854775807
        ph = ""
        for phrase in self.phrases:
            match_value = self.calculate_match_value(phrase.lower(), s.lower())
...
def main():
    if len(sys.argv) < 2:
        ...
        system_response = [
            ...
            Query(["item 1", "item i", "business", "revenue", "sales", "debt"], None, "Item 1.",),
            Query(["item 1a", "risks", "risk factors", 'litigation'], None, "Item 1A"),
            Query(["item 1b", "staff comments", "unresolved comments"], None, "Item 1B"),
            Query(["item 2", "item ii", "properties", "inventory"], None, "Item 2"),
            ...
            Query(["item 8", "financial statements", "supplementary data", "interest rates", "convertible senior notes"], None, "Item 8"),
...
def handle_user_input(line, parts, responses):
    ...
    st_dev = statistics.stdev(score_list)
    passing_score = statistics.mean(score_list) - (st_dev*2) # This is the threshold for something to print, change as wanted. Probably calculate the 2 to be closer or further
    s_list = line.split()
    if (any(word.lower() in s_list for word in ["item", "part"])): #cheaters way, i'll come back to it
        passing_score = min
    if passing_score < min:
        if passing_score + st_dev*.4 < min: #Extra .4 standard devs of leeway   
            print(line)
            print("I do not know this information")
            f.close()
            return False
        else: #I gave a little leeway so that it can print stuff that a bit less exact but it will give this message
            print("I'm not exactly sure, but my best guess is that you want: \n")
            f.write("I'm not exactly sure, but my guess is that you want: \n")
            passing_score = passing_score + st_dev*.4
    response_to_print = []
